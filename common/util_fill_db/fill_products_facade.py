...
class ManufacturerLoad():
    ...
    def __call__(self, *args, **kwargs):
        ...
        data_manufacturers = [(fake.uuid4(), uk, fake.company(), now)
                              for uk in self.manufacturers_ids]
...
class ProductLoad():
    def __init__(self,
                 cur: cursor,
                 category_data_obj: CategoryData,
                 manufacturer_load: ManufacturerLoad):
        self.cur = cur
        self.category_ids = category_data_obj.category_ids
        self.manufacturer_ids = manufacturer_load.manufacturers_ids
        self.data_images = []
    ...
    def __call__(self, *args, **kwargs):
        ...
        for num, category_id in enumerate(self.category_ids):
...
class ImageLoad():
    def __init__(self, cur: cursor, product_load_obj: ProductLoad):
        self.cur = cur
        self.data_images = product_load_obj.data_images
...
class ProductFeatureLoad():
    def __init__(self,
                 cur: cursor,
                 category_data_obj: CategoryData,
                 product_load_obj: ProductLoad):
        self.cur = cur
        self.category_feature = category_data_obj.category_feature
        self.feature_id_checkbox = category_data_obj.feature_id_checkbox
        self.feature_id_name_select = category_data_obj.feature_id_name_select
        self.product_category = product_load_obj.product_category
    ...
    def __call__(self, *args, **kwargs):
        ...
        for product_id in self.product_category.keys():
            ...
            feature_list = self.category_feature.get(category_id)
            for feature_id in feature_list:
                if feature_id in self.feature_id_checkbox:
                    data_product_feature.append(
                        (str(uuid.uuid4()), product_id, feature_id,
                         random.choice(['yes', 'no'])))
                elif feature_id in self.feature_id_name_select.keys():
                    data_product_feature.append(
                        (str(uuid.uuid4()), product_id, feature_id,
                         random.choice(
                             FEATURES_VALUE.get(
                                 self.feature_id_name_select[feature_id])
                         )))
...
class DataFakeLoad():
    """Фасад для загрузки фэйковых данных в БД"""
    def __init__(self, cur: cursor, category_data_obj: CategoryData):
        self.cur = cur,
        self.category_data_obj = category_data_obj

    def __call__(self):
        manufacturer_load_obj = ManufacturerLoad(curs)
        manufacturer_load_obj()

        product_load_obj = ProductLoad(curs, self.category_data_obj,
                                       manufacturer_load_obj)
        product_load_obj()
        images_load_obj = ImageLoad(curs, product_load_obj)
        images_load_obj()
        product_feature_load_obj = ProductFeatureLoad(curs,
                                                      self.category_data_obj,
                                                      product_load_obj)
        product_feature_load_obj()
...
if __name__ == '__main__':
    ...
    with psycopg2.connect(**DSN) as conn, conn.cursor() as curs:
        ...
        category_data_obj = download_handler()

        load_handler = DataFakeLoad(curs, category_data_obj)
        load_handler()
