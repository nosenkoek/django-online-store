...
def add_features_product(sender, instance, created, **kwargs) -> None:
    ...
    features = [ProductFeature(product_fk=instance,
                               feature_fk=feature,
                               value='-')
    ...
    if update_fields and 'category_fk' in update_fields:
        feature_product = ProductFeature.objects\
            .filter(product_fk=instance).all()
...
def add_features_product_when_update_category(sender, instance, created,
                                              **kwargs) -> None:
    ...
    product_feature = [ProductFeature(product_fk=product,
                                      feature_fk=feature,
                                      value='-')
...
def delete_features_product_when_update_category(sender, instance,
                                                 **kwargs) -> None:
    ...
    product_feature = ProductFeature.objects.filter(product_fk__in=products,
                                                    feature_fk=feature).all()
